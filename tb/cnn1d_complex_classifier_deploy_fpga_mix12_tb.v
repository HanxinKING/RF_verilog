...
module cnn1d_complex_classifier_deploy_fpga_mix12_tb;
    ...
    localparam integer TB_FLAT_STAGE_CH    = TB_STAGE_COMPLEX_CH * 2;
    ...
    task dump_conv_stage;
        input integer stage;
        integer len, ch, pos, addr;
        begin
            len = conv_len(stage);
            for (ch = 0; ch < TB_FLAT_STAGE_CH; ch = ch + 1) begin
                for (pos = 0; pos < len; pos = pos + 1) begin
                    addr = ch * len + pos;
                    $fwrite(stage_fd, "conv,%0d,%0d,%0d\n", stage, addr, dut.u_feat9.u_mem1.mem[addr]);
                end
            end
        end
    endtask

    task dump_pool_stage;
        input integer stage;
        integer len, ch, pos, addr;
        begin
            len = pool_len(stage);
            for (ch = 0; ch < TB_FLAT_STAGE_CH; ch = ch + 1) begin
                for (pos = 0; pos < len; pos = pos + 1) begin
                    addr = ch * len + pos;
                    $fwrite(stage_fd, "pool,%0d,%0d,%0d\n", stage, addr, dut.u_feat9.u_mem0.mem[addr]);
                end
            end
        end
    endtask
    ...
    task dump_sr_stage;
        ...
        begin
            ...
            for (ch = 0; ch < TB_FLAT_STAGE_CH; ch = ch + 1) begin
                for (pos = 0; pos < len; pos = pos + 1) begin
                    addr = ch * len + pos;
                    $fwrite(stage_fd, "sr,%0d,%0d,%0d\n", stage, addr, dut.u_feat9.u_mem2.mem[addr]);
                end
            end
        end
    endtask
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            for (stage_loop = 0; stage_loop < 9; stage_loop = stage_loop + 1) begin
                if (dut.u_feat9.conv_done[stage_loop] && !dumped_conv[stage_loop]) begin
                    dumped_conv[stage_loop] <= 1'b1;
                    dump_conv_stage(stage_loop + 1);
                end
                if (dut.u_feat9.bn_done[stage_loop] && !dumped_sr[stage_loop]) begin
                    dumped_sr[stage_loop] <= 1'b1;
                    dump_sr_stage(stage_loop + 1);
                end
                if (dut.u_feat9.pool_done[stage_loop] && !dumped_pool[stage_loop]) begin
                    dumped_pool[stage_loop] <= 1'b1;
                    dump_pool_stage(stage_loop + 1);
                end
            end
        end
    end
    ...
    initial begin
        ...
        stage_fd = $fopen("mem/fpga_mix12_demo/rtl_stage_dump.csv", "w");
        $fwrite(stage_fd, "kind,stage,addr,data\n");
        final_fd = $fopen("mem/fpga_mix12_demo/rtl_final.csv", "w");
        $fwrite(final_fd, "rtl_class,rtl_score,expect_class,expect_score,class_match,score_match\n");
        ...
        for (wait_cnt = 0; wait_cnt < TB_MAX_WAIT_CYCLES; wait_cnt = wait_cnt + 1) begin
            @(posedge clk);
            if (done) begin
                $fwrite(final_fd, "%0d,%0d,%0d,%0d,%0d,%0d\n",
                    out_class,
                    out_score,
                    expect_class_mem[0],
                    expect_score_mem[0],
                    (out_class == expect_class_mem[0]),
                    (out_score == expect_score_mem[0])
                );
                ...
                if (out_class == expect_class_mem[0] && out_score == expect_score_mem[0]) begin
                    $display("TEST PASS: cnn1d_complex_classifier_deploy_top_fpga_mix12");
                end else begin
                    $display("TEST FAIL: cnn1d_complex_classifier_deploy_top_fpga_mix12");
                end
                $fclose(stage_fd);
                $fclose(final_fd);
                $finish;
